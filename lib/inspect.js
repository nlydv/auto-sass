...
async function checkFirstLine(input) {
    ...
    if ( firstLine.startsWith("/") ) {
        ...
        const [ key, value ] = firstLine.substr(2).split(":").map(i => i.replace(/(\*\/|\/\/)/, "").trim());
        ...
        switch (key) {
            ...
            case "compiles":
                args.output = value;
                args.output = args.output.replace("$1", name).replace("_", "");
            ...
            case "downstreams":
                ...
                args.downstream = args.downstream.map(i => i.replace("$1", name).replace("_", ""));
